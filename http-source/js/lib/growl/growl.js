...
Gr0wl.Base = Class.create({
    ...
    show: function(options) {
        ...
        elements.each(function(e, i) {
            ...
            if(i) {
            	var h3 = e.down(0);
            	h3.update(options.title).next().update(options.text);
            }
        });
...
Gr0wl.Smoke = Class.create(Gr0wl.Base, {
	...
	show: function($super, options) {
		...
		options = Object.extend(options, {
            position: {'top':delta+'px', 'right':'12px', 'display':'block'},
            queue: 'parallel'
        }); 
...
var Growl =  {
    initialize: function() {
        this.oBezel = new Gr0wl.Bezel('/js/lib/growl/images/bezel.png');
        this.Bezel =  this.oBezel.show.bind(this.oBezel);
        this.oSmoke = new Gr0wl.Smoke('/js/lib/growl/images/smoke.png');
